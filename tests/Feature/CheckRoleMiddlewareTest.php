...
class CheckRoleMiddlewareTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        Route::middleware(['web', 'tenant', 'role:staff'])
            ->get('/test/staff', fn () => response()->json(['ok' => true]));
    ...
    {
        [, $user] = $this->member('staff');

        $this->actingAs($user)->get('/test/staff')->assertOk();
    }

    public function test_a_higher_role_satisfies_a_lower_requirement(): void
    {
        [, $user] = $this->member('org_admin');
    ...
    public function test_a_lower_role_is_rejected(): void
    {
        ...
        $this->actingAs($user)->get('/test/staff')->assertForbidden();
    }

    public function test_an_org_admin_cannot_reach_an_owner_only_route(): void
    {
        [, $user] = $this->member('org_admin');
    ...
    {
        $this->get('/test/staff')->assertUnauthorized();
    }
    ...
    public function test_the_organization_header_selects_the_tenant(): void
    {
        ...
        // second organization is not enough for a staff route.
        $this->actingAs($user)
